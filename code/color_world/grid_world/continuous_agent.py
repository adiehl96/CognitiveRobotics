import math
from code.color_world.grid_world.agent import Agent
...
class ContinuousAgent(Agent):
    def go_in_direction(self, dir, distance=1, return_obstacle=False):
        ...
        dx1, dy1 = self.world.get_offset_in_direction(
            self.cell.x, self.cell.y, dir1
        )
        dx2, dy2 = self.world.get_offset_in_direction(
            self.cell.x, self.cell.y, dir2
        )
        ...
        for n in self.world.get_cell_neighbours(self.cell):
            d = (x - n.x) ** 2 + (y - n.y) ** 2
            if d < dist:
                closest = n
                dist = d
    ...
    def detect(self, direction, max_distance=None):
        ...
        while distance < max_distance:
            obstacle = self.go_in_direction(
                direction, delta, return_obstacle=True
            )
            if obstacle is None:
                distance += delta
                ...
                distance = math.sqrt(
                    (start_x - self.x) ** 2 + (start_y - self.y) ** 2
                )
                break
        self.cell = cell
        self.x = start_x
