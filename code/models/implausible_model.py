...
def setup_implausible_model(model, color_world):
    ...
    with model:
        env = grid.GridNode(world, dt=0.005, label='environment')
        ...
        pau = 1
        nengo.Connection(green_integrator, accumulator,
            transform=pau, synapse=tau)
        nengo.Connection(red_integrator, accumulator,
            transform=pau, synapse=tau)
        nengo.Connection(blue_integrator, accumulator,
            transform=pau, synapse=tau)
        nengo.Connection(magenta_integrator, accumulator,
            transform=pau, synapse=tau)
        nengo.Connection(yellow_integrator, accumulator,
            transform=pau, synapse=tau)
